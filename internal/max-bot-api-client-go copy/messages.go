...
func (a *messages) sendMessage(ctx context.Context, reset bool, chatID int64, userID int64, message *schemes.NewMessageBody) (*schemes.Message, error) {
	...
	format := ""
	if message != nil {
		attachmentsCount = len(message.Attachments)
		textLen = len(message.Text)
		format = message.Format
	}
	if isDebugLogsEnabled() {
		log.Printf("max send start: chat_id=%d user_id=%d text_len=%d attachments=%d format=%q", chatID, userID, textLen, attachmentsCount, format)
	}

	body, err := a.client.request(ctx, http.MethodPost, pathMessages, values, reset, message)
	if err != nil {
		if isDebugLogsEnabled() {
			log.Printf("max send failed: chat_id=%d user_id=%d err=%v", chatID, userID, err)
		}
		return nil, err
	}
	defer func() {
	...
	if err := json.NewDecoder(body).Decode(wrapper); err != nil {
		if isDebugLogsEnabled() {
			log.Printf("max send decode failed: chat_id=%d user_id=%d err=%v", chatID, userID, err)
		}
		return nil, err
	}
	if isDebugLogsEnabled() {
		log.Printf("max send success: chat_id=%d user_id=%d mid=%s", chatID, userID, wrapper.Message.Body.Mid)
	}
